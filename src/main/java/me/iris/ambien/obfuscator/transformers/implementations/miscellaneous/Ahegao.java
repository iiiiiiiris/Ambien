...
public class Ahegao extends Transformer {
    ...
    public final StringSetting varName = new StringSetting("var-name", "nothing_to_see_here");
    public final StringSetting copyPastesFile = new StringSetting("copy-pastes-file", "");
    public final BooleanSetting annotationClasses = new BooleanSetting("annotations-classes", true);
    public final BooleanSetting annotationFields = new BooleanSetting("annotations-fields", true);
    public final BooleanSetting annotationMethods = new BooleanSetting("annotations-methods", true);

    @Override
    public void transform(JarWrapper wrapper) {
        if(!copyPastesFile.getValue().equals(""))
            ahegaos = StringUtil.readCopyPastes(copyPastesFile.getValue());
        Ambien.LOGGER.info("Ahegaos count: " + ahegaos.size());

        getClasses(wrapper).stream()
                .filter(classWrapper -> !classWrapper.isEnum() && !classWrapper.isInterface() && !classWrapper.isAnnotation())
        ...
        classMethodsMap.forEach((classWrapper, methods) -> {
            if (!varName.getValue().equals(""))
                injectAhegaoClinit(classWrapper);
            injectAhegaoAnnotations(classWrapper);
        });
    }

    public void injectAhegaoAnnotations(ClassWrapper wrapper) {
        if (annotationClasses.isEnabled()) {
            final Random random = new Random();
            final String[] array = ahegaos.get(random.nextInt(ahegaos.size()));

            StringBuilder combinedAnnotations = new StringBuilder();
            for (String s : array) {
                combinedAnnotations.append("\n").append(s);
            }

            String repeatedEndLine = combinedAnnotations.toString();
        ...
        if (annotationMethods.isEnabled()) {
    ...
    public void injectAhegaoClinit(ClassWrapper wrapper) {
        final String[] array = ahegaos.get(new Random().nextInt(ahegaos.size()));
        final String name = varName.getValue();
        ...
        clinitInstructions.add(new TypeInsnNode(Opcodes.ANEWARRAY, "java/lang/String"));
        clinitInstructions.add(new FieldInsnNode(Opcodes.PUTSTATIC, wrapper.getNode().name, name, "[Ljava/lang/String;"));
        for (int i = 0; i < array.length; i++) {
            clinitInstructions.add(new FieldInsnNode(Opcodes.GETSTATIC, wrapper.getNode().name, name, "[Ljava/lang/String;"));
            clinitInstructions.add(new IntInsnNode(SIPUSH, i));
            clinitInstructions.add(new LdcInsnNode(array[i]));
